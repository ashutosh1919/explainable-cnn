...
class CNNValidator(BaseValidator):
    def __init__(self):
        """
        Validations corresponding to CNN related features used in order to generate
        explanations.
        """
        pass
    ...
    def assert_layers(self, layer_names, model):
        ...
        if len(copied_layers) != 0:
            raise ValueError(f"Some layers you passed doesn't exist in the model: {copied_layers}")
    
    def assert_input_shape(self, input_shape):
        if not isinstance(input_shape, tuple) and not isinstance(input_shape, list):
    ...
    def assert_valid_image_path(self, image_path):
        ...
        if not os.path.isfile(image_path):
            raise FileNotFoundError(f"File with given path does not exist: {image_path}")
        if not any([image_path.endswith(x) for x in valid_image_types]):
            raise ValueError("Image type not supported yet.",
                f"Please pass one of following type: {valid_image_types}")
    ...
    def assert_valid_transforms(self, transforms):
        if not isinstance(transforms, torchvision.transforms.Compose) and \
            ...
            raise ValueError("Attribute transform must be either None or of",
